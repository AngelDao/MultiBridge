...
const checkIfApprovedToBridge = async (token, approvedFor, provider) => {
    ...
    let checkAllowance = await contractWithSigner.allowance(myAddress, approvedFor)
...
const getApproval = async (token, approveFor, provider) => {
    ...
    let approveAllowance = await contractWithSigner.approve(approveFor, approve_amount)
...
*/
        ...
        const inputdata = [ callData[i][0], callData[i][1], myAddress, ethers.utils.parseEther(callData[i][2].toString()).toString(), 500000, "0x"  ]
